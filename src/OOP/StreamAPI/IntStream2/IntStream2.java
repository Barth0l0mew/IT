package OOP.StreamAPI.IntStream2;
...
//Вывести первые n чисел Фибоначчи
        ...
        int[] fibonachi = Stream.iterate(new int[]{0,1},t->new int[]{t[1],t[0]+t[1]})
                .limit(10)
                .mapToInt(t->t[0])
                .toArray();
        System.out.println("Exercise 1 \n"+ Arrays.toString(fibonachi));
        //Сумма четных чисел Фибоначчи, не превышающих count
        int sumFibEven = Stream.iterate(new int[]{0,1},t->new int[]{t[1],t[0]+t[1]})
                .filter(t->t[0]%2==0)
                .takeWhile(t->t[0]<100)
                .mapToInt(t->t[0])
                .sum();
...
//Вывести первые n факториалов
        int[] factorial = Stream.iterate(new Pair(1,1),x->new Pair(x.num+1,x.value*(x.num+1)))
                .limit(6)
                .mapToInt(x-> x.value)
                .toArray();
        System.out.println("Exercise 3 \n"+ Arrays.toString(factorial));
