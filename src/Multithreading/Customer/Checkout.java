package Multithreading.Customer;
...
public class Checkout {
    static AtomicInteger numCustomer = new AtomicInteger(1);
        ...
        public void run() {
            while (true) {
                synchronized (mutex) {
                    ...
                    System.out.printf("Продавец рассчитал сумму товара покупателя: %d, сумма: %f%n", customer.id, customer.sum);
                    mutex.notify();
                    try {
                        mutex.wait();
                    } catch (InterruptedException e) {
